...
import { useNamedQueue, useLastGoodQueueItem } from '../zust-queue/useQueue'
...
const generateModel = async (overrides, { abortSignal }) => {
  ...
  console.log(
    `--> Fancy Worker: worker created in ${timeAfterWorkerCreated -
      timeAfterWorkerKilled}ms`
  )
  ...
  console.log(
    `--> Fancy Worker: model generated in ${timeAfterModelGenerated -
      timeAfterWorkerCreated}ms with overrides = ${JSON.stringify(overrides)}`
  )
  ...
  console.log(
    `--> Fancy Worker: converted model to threejs in ${end -
      timeAfterModelGenerated}ms`
  )
