...
export const useQueueStore = create((set) => ({
  queues: {},
  add: (
    fn: QueueItemFunction,
    config: QueueConfig,
    params: QueueItemFunctionOverrides = {}
  ) => {
    ...
    const setError = (index, error) => {
      set(
        produce((state) => {
          state.queues[queueKey][index].error = error;
        })
      );
    };
...
const isResolved = (item) =>
  !!(item && (item.error || item.aborted || item.response));
...
export type QueueItemFunction = (
  overrides: QueueItemFunctionOverrides,
  config: QueueItemFunctionConfig
) => Promise<any>;
...
export interface QueueItemFunctionOverrides {}
export interface QueueItemFunctionConfig {
  abortSignal: AbortSignal;
}

export const useNamedQueue = (
  fn: QueueItemFunction,
  config: QueueConfig,
  params
) => {
